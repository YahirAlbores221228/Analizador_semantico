...
tokens = (
    'INT', 'SEMICOLON', 'ID', 'EQUALS', 'NUMBER', 'FOR', 'LPAREN', 'RPAREN', 'LT', 'GT', 'LE', 'GE', 'EQ', 'NE', 'PLUSPLUS', 'LBRACE', 'RBRACE', 'PRINTLN'
)
...
def p_condition(p):
    ...
    var_name = p[1]
    if var_name not in variables:
        errors.append(f"Línea {p.lineno(1)}: La variable '{var_name}' usada en la condición no está declarada.")
    else:
        p[0] = True
...
def p_increment(p):
    ...
    var_name = p[1]
    if var_name not in variables:
        errors.append(f"Línea {p.lineno(1)}: La variable '{var_name}' usada en el incremento no está declarada.")
    else:
        p[0] = True
